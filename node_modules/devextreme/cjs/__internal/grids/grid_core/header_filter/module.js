/**
 * DevExtreme (cjs/__internal/grids/grid_core/header_filter/module.js)
 * Version: 23.1.3
...
function convertDataFromUTCToLocal(data, column) {
    ...
    var group = _module_utils.default.getHeaderFilterGroupParameters(_extends(_extends({}, column), {
...
var HeaderFilterController = _modules.default.ViewController.inherit({
    init: function() {
    ...
    _updateSelectedState: function(items, column) {
        ...
        while (i--) {
            ...
            }(0, _module_core.updateHeaderFilterItemSelectionState)(item, _module_utils.default.getIndexByKey(items[i].value, column.filterValues, null) > -1, isExclude)
        }
    },
    ...
    getHeaderItemText: function(displayValue, column, currentLevel, headerFilterOptions) {
        var text = _module_utils.default.formatValue(displayValue, function(value, column, currentLevel) {
            var groupInterval = _filtering.default.getGroupInterval(column);
            var result = _module_utils.default.getFormatOptionsByColumn(column, "headerFilter");
            if (groupInterval) {
                result.groupInterval = groupInterval[currentLevel];
                if (_module_utils.default.isDateType(column.dataType)) {
                    ...
                    result.getDisplayFormat = function() {
                        ...
                        var firstValueText = _module_utils.default.formatValue(value, formatOptions);
                        var secondValue = value + groupInterval[currentLevel];
                        var secondValueText = _module_utils.default.formatValue(secondValue, formatOptions);
    ...
    getDataSource: function(column) {
        ...
        var group = _module_utils.default.getHeaderFilterGroupParameters(column, remoteGrouping);
        ...
        if ((0, _type.isDefined)(headerFilterDataSource) && !(0, _type.isFunction)(headerFilterDataSource)) {
            ...
            if (this.option("syncLookupFilterValues")) {
                ...
                options.dataSource = _module_utils.default.getWrappedLookupDataSource(column, dataSource, filter)
            } else {
                options.dataSource = _module_utils.default.normalizeLookupDataSource(column.lookup)
            }
        } else {
...
var ColumnHeadersViewHeaderFilterExtender = (0, _extend.extend)({}, _module_core.headerFilterMixin, {
    _renderCellContent: function($cell, options) {
        var that = this;
        var $headerFilterIndicator;
        var column = options.column;
        if (!column.command && (0, _module_core.allowHeaderFiltering)(column) && that.option("headerFilter.visible") && "header" === options.rowType) {
    ...
    _columnOptionChanged: function(e) {
        ...
        if (_module_utils.default.checkChanges(optionNames, ["filterValues", "filterType"])) {
...
var HeaderPanelHeaderFilterExtender = (0, _extend.extend)({}, _module_core.headerFilterMixin, {
    _createGroupPanelItem: function($rootElement, groupColumn) {
        var that = this;
        var $item = that.callBase.apply(that, arguments);
        var $headerFilterIndicator;
        if (!groupColumn.command && (0, _module_core.allowHeaderFiltering)(groupColumn) && that.option("headerFilter.visible")) {
            $headerFilterIndicator = that._applyColumnState({
                name: "headerFilter",
                rootElement: $item,
                column: {
...
var DataControllerFilterRowExtender = {
    ...
    _calculateAdditionalFilter: function() {
        ...
        (0, _iterator.each)(columns, (function(_, column) {
            ...
            if ((0, _module_core.allowHeaderFiltering)(column) && column.calculateFilterExpression && Array.isArray(column.filterValues) && column.filterValues.length) {
                var filterValues = [];
                (0, _iterator.each)(column.filterValues, (function(_, filterValue) {
                    if (Array.isArray(filterValue)) {
                        filter = filterValue
                    } else {
                        if (column.deserializeValue && !_module_utils.default.isDateType(column.dataType) && "number" !== column.dataType) {
                ...
                filterValues = _module_utils.default.combineFilters(filterValues, "or");
...
var headerFilterModule = {
    ...
    views: {
        headerFilterView: _module_core.HeaderFilterView
    },
    extenders: {
        controllers: {
